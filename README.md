...
Every time you run `sigmaforage`, you'll see the Sigma Forage banner, then your command runs:
...
**Output (valid Splunk SPL—paste into Splunk Search):**
...
- **CLI-first:** Convert from the command line; path to rule + SIEM = native query.
...
## Pushing this repo to GitHub (SigmaForage)

1. **Create a new repository on GitHub**
   - Go to [github.com/new](https://github.com/new).
   - Repository name: **SigmaForage**
   - Description: *One Sigma rule. Every SIEM. Convert Sigma rules to native SIEM queries.*
   - Do **not** initialize with a README, .gitignore, or license (this repo already has them).

2. **From your local SigmaForage folder (this folder):**

```bash
cd ~/Desktop/sigma-forge
git init
git add .
git commit -m "Initial commit: SigmaForage — Sigma to SIEM query conversion"
git branch -M main
git remote add origin https://github.com/aakashraman/SigmaForage.git
git push -u origin main
```

3. Replace `aakashraman` with your GitHub username if different. After pushing, your repo will be live at `https://github.com/aakashraman/SigmaForage`.
